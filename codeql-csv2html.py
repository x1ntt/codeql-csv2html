...
def gen_info_tips(rule_num, rule_type_num):
    tmp_str = "触发<b>"+str(rule_type_num)+"</b>条规则 共<b>"+ str(rule_num) +"</b>个报告，文件生成时间：" + time.strftime('%Y年%m月%d日 %H:%M:%S ',time.localtime(time.time()));
    return tmp_str
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("将cvs文件渲染到template.html中")
        print("usage: ./" + sys.argv[0] + " template.html file.csv target.html")
        exit()
